...
(function($) {
  $(document).ready( function (){
    $('.physicians-find-a-physicians-collapsible').collapsibleSelector({
      childLeftOffset: 15
    });
  });
  $.fn.collapsibleSelector = function(option) {
    ...
    this.find('ul ul span').bind('click', function(){
      clickAction(this, 'slide');//for 3d lvl element and more - effect hide
    });
    return this;
...
(function($) {
  $(function(){
    ...
    var $el = $('.middle-content .block-content-item.with-border')
    $el.last().removeClass('with-border');
