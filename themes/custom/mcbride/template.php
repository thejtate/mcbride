...
function mcbride_preprocess_page(&$vars, $hook) {
    ...
    if (!empty($vars['node'])) {
        ...
        switch ($node->type) {
            ...
            case 'physician':
                ...
                $links = (!empty($vars['tabs']) && !empty($vars['tabs']['#primary'])) ? $vars['tabs']['#primary'] : array();
    ...
    if (in_array('page__news', $vars['theme_hook_suggestions'])) {
      ...
      if(user_access('edit any header_config_view_page content')){
        $variables = array('items' => array(0 => array(l(t('Edit'),'node/' . HEADER_NODE_FOR_VIEW_NEWS_NID . '/edit', array('query' => array('destination' => 'news')))), 1 => array(l(t('Add'),'node/add/news', array('query' => array('destination' => 'news'))))),'type' => 'ul','attributes' => array('class' => array('tabs', 'primary', 'clearfix')));
    ...
    if (in_array('page__find_a_physicians', $vars['theme_hook_suggestions'])) {
      if(user_access('edit any header_config_view_page content')){
        $variables = array('items' =>
            ...
            3 => array(l(t('Rearrange Physicians'),'admin/structure/nodequeue/4/view/4', array('query' => array(drupal_get_destination())))),
            4 => array(l(t('Rearrange Locations'),'admin/structure/taxonomy/location', array('query' => array(drupal_get_destination()))))),'type' => 'ul','attributes' => array('class' => array('tabs', 'primary', 'clearfix')));
       $vars['tabs']= theme("item_list", $variables);
      }
	...
	$vars['page']['calltoaction'] = _mcbride_calltoaction();
  ...
  $vars['footer_logo'] = theme('image', array('path' => path_to_theme() . '/images/DNV-GL_accredited_hospital.png'));
  $vars['footer_logo2'] = theme('image', array('path' => path_to_theme() . '/images/rc-logo-s.png'));
  $footer_logo_link = variable_get('footer-logo-link', FALSE);
  if (!empty($footer_logo_link)) {
    $vars['footer_logo'] = l($vars['footer_logo'], $footer_logo_link, array('html' => TRUE, 'external' => TRUE));
  }


  $vars['footer_logo_link'] = variable_get('footer-logo-link', '#');
  $footer_file_fid = variable_get('mcbride_custom_footer_file_fid', FALSE);
  $footer_file_title = variable_get('mcbride_custom_footer_file_title', '');
  $footer_file = $footer_file_fid ? file_load($footer_file_fid) : FALSE;
  if ($footer_file) {
    $filename = preg_replace("/\.[^.]+$/", "", $footer_file->filename);
    $footer_file_title = $footer_file_title ? $footer_file_title : $filename;
    $file_url = file_create_url($footer_file->uri);
    $vars['footer_file'] = l($footer_file_title, $file_url, array('attributes' => array('target'=>'_blank')));
  }
}
/**
...
function mcbride_preprocess_node(&$vars, $hook) {
    ...
    switch ($vars['type']) {
      case 'homepage_slider_link':

        if(!empty($vars['field_homepage_slider_vimeo'][LANGUAGE_NONE][0]['file']->uri) &&
          $vars['field_homepage_slider_vimeo'][LANGUAGE_NONE][0]['file']->filemime == 'video/vimeo') {

          $wrapper = file_stream_wrapper_get_instance_by_uri($vars['field_homepage_slider_vimeo'][LANGUAGE_NONE][0]['file']->uri);
          $parts = $wrapper->get_parameters();
          $video_id = $parts['v'];
          $html_id = drupal_html_id('vimeo_iframe');
          $vars['vimeo_iframe'] = array(
            '#markup' => '<a href="#' . $html_id . '" class="watch-vimeo-link read-more">WATCH VIDEO</a><div class="vimeo-iframe-wrapper">' . mcbride_vimeo_iframe($video_id, $html_id) . '</div>',
          );
        }
        break;
        case 'careers':
        ...
        case 'physician':
            ...
            $playlist = array();
            $video_list = (!empty($vars['node']->field_physician_video) && !empty($vars['node']->field_physician_video[LANGUAGE_NONE])) ?
              $vars['node']->field_physician_video[LANGUAGE_NONE] : array();
            foreach ($video_list as $key => $val) {
              if ($val['file']->filemime == 'video/youtube'){
                ...
                $vimeo_url = "http://vimeo.com/api/v2/video/" . str_replace('vimeo://v/','',$val['file']->uri) . ".php";
                $result = drupal_http_request($vimeo_url, array('method' => 'GET',));
                $vimeo_thumbnail = '';

                if (!empty($result) && ($result->code == 200)) {
                  $vimeo_data = unserialize($result->data);
                  if (!empty($vimeo_data[0]) && !empty($vimeo_data[0]['thumbnail_small'])) {
                    $vimeo_thumbnail = $vimeo_data[0]['thumbnail_small'];
                  }
                }
 ...
 * @return string
 */
function _mcbride_calltoaction() {
    $content = '';
    $calltoaction = variable_get('calltoaction');
    if (!empty($calltoaction)) {
        $links = array(
            'title' => t('Configure call to action'),
            'path' => 'admin/config/system/site-information',
        );
        if (user_access('access manager links')) {
            $content .= theme('manager_links', array('links' => $links));
...
function _mcbride_social_icons() {
    ...
    if (user_access('access manager links')) {
      $content .= theme('manager_links', array('links' => '', 'arr_links' => $links));
    }
...
function mcbride_menu_structure($items) {
    ...
    foreach ($items as $link) {
        if (isset($link['link']['hidden']) && !$link['link']['hidden']) {
            $sub_menu = '';
            $ot = array(0, 0);
...
function _mcbride_count_not_hidden_menu_items($items) {
    ...
    foreach ($items as $key => $value) {
        if (isset($value['link']['hidden']) && !$value['link']['hidden']) {
            ++$result;
        }
    }
    return $result;
 ...
 */
function mcbride_form_alter(&$form, &$form_state, $form_id) {
//  kpr($form_id);
//  kpr($form);
}

function mcbride_vimeo_iframe($video_id, $html_id) {
  return "
<div id='$html_id'><iframe src=\"https://player.vimeo.com/video/$video_id\" width=\"640\" height=\"360\"
        frameborder=\"0\" webkitallowfullscreen
        mozallowfullscreenallowfullscreen></iframe></div>
  ";
}